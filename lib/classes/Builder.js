...
export class Builder extends NativeLoader {
    ...
    /**
     * Configures the [Node] instance to source liquidity through JIT channels from an LSPS2 compatible provider.
     * @requires address
     * @requires publicKey
     * @requires token
     */
    async setLiquiditySourceLsps2(address, publicKey, token) {
        return await this._ldk.setLiquiditySourceLsps2(this.id, addressToString(address), publicKey, token);
    }
    /**
